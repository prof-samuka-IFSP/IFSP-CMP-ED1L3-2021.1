...
LinkedList *LinkedList_copy(const LinkedList *L) {
    LinkedList *L_copy = LinkedList_create();

    SNode *p = L->begin;

    while (p != NULL) {
        LinkedList_add_last(L_copy, p->val);
        p = p->next;
    }
...
void LinkedList_erase(LinkedList *L) {
    SNode *p = L->begin;
    SNode *aux = NULL;
...
int LinkedList_sum(const LinkedList *L) {
    int sum = 0;
    SNode *p = L->begin;
